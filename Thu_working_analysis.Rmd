...
for (i in gain_ind) {
  ...
  # store variable importances as a data frame, will have list of data frames at the end
  varimp_y1[[name]] <- as.data.frame(varImp(rf_fit)$importance)
}
```


```{r}
# need to eventually have something that'll tally up the most xx important variables (check i with Jonathan)
```

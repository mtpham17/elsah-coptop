...
```{r comparison year1, echo=TRUE, fig.height=8, cache=TRUE, eval=FALSE}
...
for (outcome in outcomes) {
  ...
  comparison_df <- rbind(comparison_df, cbind(x, overlap, rep(outcome, length(x))))
}

comparison_df <- comparison_df %>% 
  rename("lasso" = "x", "rf" = "overlap", "outcome" = "V3") %>%

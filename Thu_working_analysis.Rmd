...
y1_child_obs <- y1_child_obs_raw %>%
  ...
  # hypothesis that some of these are throwing NaNs due to float issues and square root 
  # of negative numbers -- use variance instead of sd
  mutate(across(starts_with("o_c") & !ends_with("classmean"), ~ sqrt((sum((.x - get(str_c(cur_column(), '_classmean')))^2) - (.x - get(str_c(cur_column(),'_classmean')))^2) / get('nclass')), .names = "{col}_classsd")) %>%
  ungroup
